...
def test_disabled_model_route_blocks_in_preflight_without_crashing():
    ...
    assert "no execution adapter configured" in response.reason


def test_realtime_voice_route_reports_clear_unavailable_reason():
    response = run_preflight(_request(task_type=TaskType.realtime_voice))
    assert response.allowed is False
    assert response.recommended_model == "gemini-live"
    assert response.fallback_model is None
    assert "model_unavailable" in response.stop_rules
    assert "disabled in models.yaml" in response.reason
